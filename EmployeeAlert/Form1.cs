...
namespace AlertFormV2
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            log.Log("Starting Program", LoggerLogLevel.Info);
            DB_Connect(1);
            DB_Close();
            InitializeComponent();
        ...
        {
            //uef.Hide();
            //uef.ShowInTaskbar = true;
            if (tmrPersist.Interval > 0)
            {
                tmrPersist.Enabled = true;
                tmrPersist.Start();
            } 
        }
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (cmbNames.SelectedIndex != -1)
            {
                if (isImmediate)
                {
                    ...
                    // Update Events set Inactive = 1, InactiveDate = '" + DateTime.Now + "' where EV_ID = '" + immEvID + "'"
                }
                tmrPersistAfterAck.Enabled  = true;
                //this.lblMessage.Text        = this.lblMessage.Text.Trim() + " - Acknowledged";
        ...
        public void initializer()
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                {
                    //DateTime dw = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, int.Parse(row["TimeToRun"].ToString().Substring(0, 2)), int.Parse(row["TimeToRun"].ToString().Substring(3, 2)), int.Parse(row["TimeToRun"].ToString().Substring(6, 2)));
            ...
            tmrDriver.Start();
            tmrUpdate.Start();
            log.Log("initial " + dateDict.Count, LoggerLogLevel.Info);
        }
            ...
            foreach (DataRow row in this.table.Rows)
            {
                ...
                else if (row["Frequency"].ToString() == "H")
                {
                    if (DateTime.Now.ToLongTimeString() == dateDict[int.Parse(row["EV_ID"].ToString())].ToLongTimeString())
                    {
                        ...
                        {
                            Form1 frm = new Form1();
                            //if (int.Parse(row["DisplayLocation"].ToString()))
                            if (this.Top > 0)
                            {
                                frm.Top = 0;
                            }
        ...
        private void TmrUpdate_Tick(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                else if (row["Frequency"].ToString() == "H" && bool.Parse(row["EventsInactive"].ToString()) == false)
                {
                    if (DateTime.Now.Minute < int.Parse(row["HourlyAtMinute"].ToString()))
                    {
                        dateDict.Add(int.Parse(row["EV_ID"].ToString()), new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour, int.Parse(row["HourlyAtMinute"].ToString()), 0));
                        uef.lbUpcoming.Items.Add("Name:" + row["EV_Description"].ToString() + "\t\t\t  Frequency: " + row["Frequency"].ToString() + "\t\t\t\t  Next Notification: " + new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour, int.Parse(row["HourlyAtMinute"].ToString()), 0));
                        log.Log("updated time: " + new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour, int.Parse(row["HourlyAtMinute"].ToString()), 0), LoggerLogLevel.Info);
                    }
                    else
                    {
                        dateDict.Add(int.Parse(row["EV_ID"].ToString()), new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour + 1, int.Parse(row["HourlyAtMinute"].ToString()), 0));
                        uef.lbUpcoming.Items.Add("Name:" + row["EV_Description"].ToString() + "\t\t\t  Frequency: " + row["Frequency"].ToString() + "\t\t\t\t  Next Notification: " + new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour + 1, int.Parse(row["HourlyAtMinute"].ToString()), 0));
                        log.Log("updated time: " + new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour + 1, int.Parse(row["HourlyAtMinute"].ToString()), 0), LoggerLogLevel.Info);
                    }
                    k++;
                }
                //}
            ...
            log.Log("update Workstations set LastActivity = '" + DateTime.Now.ToString() + "' where WS_Computer_Name = '" + Environment.MachineName + "'", LoggerLogLevel.Info);
